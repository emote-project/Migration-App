...
public class Max3DSParser extends AParser implements IParser {
	...
	//NOTE  0x4160 is local co-ordinates
	private final int LOCAL_COORD = 0x4160;
	...
	private void readLocalCoords(InputStream buffer)throws IOException {
	
		for (int i = 0; i < 9;++i) {
			//System.out.println(" RLC VALUE: "+ readFloat(buffer));
			readFloat(buffer);
		}
		//last 3 are X,Y,Z in max, so need to flip axes etc
		float x, y, z, tmpy;
		x = readFloat(buffer);
        y = readFloat(buffer);
        z = readFloat(buffer);
	...
	public Object3dContainer getParsedObject() {
		...
		for (int i = 0; i < numObjects; i++) {
			...
			obj.addChild(new Object3dContainer(o.getParsedObject(materialMap, textureAtlas)));
		}
